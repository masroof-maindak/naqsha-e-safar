...
def geocode_location(location_name: str):
    ...
    Uses an in-memory cache for the lifetime of the process.\"""
    ...
    try:
        ...
        if loc:
            result = [loc.longitude, loc.latitude]  # type: ignore
...
def create_route_feature(route_name, route_data, route_id):
    ...
    # TODO: Get rid of this bullshit assumption (not mine by the way; collective accountability
    # be damned) and properly calculate displacement b/w stops using their lat-long
    length = len(coordinates) * 500
...
def write_geojson():
    ...
    with open(SPEEDO_GEOJSON_FPATH, "w", encoding="utf-8") as f:
        json.dump(collection, f, indent=2, ensure_ascii=False)

    print(f"\n GeoJSON written to {SPEEDO_GEOJSON_FPATH}")
    print(f" Total routes: {len(features)}")


if not os.path.exists(SPEEDO_GEOJSON_FPATH):
...
# NOTE: we previously used the aforementioned code block and the following JSON
# to query and save Speedo route along w/ their stops. Quite a few nodes were
# missed though and the data in general was kind of low-quality so we scrapped
# the above, manually verified/fixed the output we did get, and chucked it into
# `speedo_routes_hand_cleaned.csv`
